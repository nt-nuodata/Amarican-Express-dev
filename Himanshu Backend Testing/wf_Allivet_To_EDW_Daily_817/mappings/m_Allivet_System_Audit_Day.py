...
df_5=spark.sql("""
    SELECT
        ...
        OR ISNULL(Lkp_ALLIVET_ORDER_CNTR)) 
        AND (IFF(ISNULL(ALLIVET_ORDER_LINE_NUMBER),
        -1,
        ALLIVET_ORDER_LINE_NUMBER) <> IFF(ISNULL(Lkp_ALLIVET_ORDER_LN_NBR),
        -1,
        Lkp_ALLIVET_ORDER_LN_NBR) 
        OR IFF(ISNULL(OPERATION),
        '0',
        OPERATION) <> IFF(ISNULL(LKP_OPERATION),
        '0',
        LKP_OPERATION) 
        OR IFF(ISNULL(CUSTOMER_CODE),
        '0',
        CUSTOMER_CODE) <> IFF(ISNULL(LKP_ALLIVET_CUSTOMER_NBR),
        '0',
        LKP_ALLIVET_CUSTOMER_NBR) 
        OR IFF(ISNULL(EXTRA_INFO),
        '0',
        EXTRA_INFO) <> IFF(ISNULL(LKP_EXTRA_INFO),
        '0',
        LKP_EXTRA_INFO) 
        OR IFF(ISNULL(CHANGE_DATE),
        TO_DATE('9999-12-31',
        'YYYY-MM-DD'),
        CHANGE_DATE) <> IFF(ISNULL(LKP_ORDER_CHG_DT),
        TO_DATE('9999-12-31',
        'YYYY-MM-DD'),
        LKP_ORDER_CHG_DT) 
        OR IFF(ISNULL(FROM_ITEM_CODE),
        '0',
        FROM_ITEM_CODE) <> IFF(ISNULL(LKP_FROM_ITEM_CD),
        '0',
        LKP_FROM_ITEM_CD) 
        OR IFF(ISNULL(TO_ITEM_CODE),
        '0',
        TO_ITEM_CODE) <> IFF(ISNULL(LKP_TO_ITEM_CD),
        '0',
        LKP_TO_ITEM_CD) 
        OR IFF(ISNULL(PRESCRIPTION_ID),
        -1,
        PRESCRIPTION_ID) <> IFF(ISNULL(LKP_PRESCRIPTION_ID),
        -1,
        LKP_PRESCRIPTION_ID)),
        2,
        0)) AS UPDATE_FLAG,
